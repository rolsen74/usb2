...
static struct OpenStruct OpenList[] =
{
	...
	{ OT_Class,			"gadgets/listbrowser.gadget",	50,		& ListBrowserBase,	& ListBrowserClass },
	...
	{ OT_Class,			"window.class",					50,		& WindowBase,		& WindowClass },
	{ OT_Interface,		"main",							1,		& ListBrowserBase,	& IListBrowser },
...
//	MYERROR( "USB2Analyzer : myOpenResources" );
	...
	for( pos=0 ; pos < max_libs ; pos++ )
	{
		...
		switch( OpenList[pos].os_Type )
		{
			case OT_Library:
			{
				...
				if ( ! base )
				{
					printf( "USB2Analyzer : Error missing base pointer" );
					goto bailout;
				}
...
//					MYINFO( "USB2Analyzer : Opening library '%s' v%lu", name, vers );

					ptr = OpenLibrary( name, vers );

					if ( ! ptr )
					{
						printf( "USB2Analyzer : Error opening library '%s' v%u", name, vers );
						goto bailout;
					}
			...
			case OT_Interface:
			{
				...
				if ( ! ifc )
				{
					printf( "USB2Analyzer : Error missing interface pointer" );
					goto bailout;
				}
...
//					MYINFO( "USB2Analyzer : Getting interface pos #%lu", pos );

					ptr = GetInterface( *base, name, vers, NULL );

					if ( ! ptr )
					{
						printf( "USB2Analyzer : Error getting interface pos #%u", pos );
						goto bailout;
					}
			...
			case OT_Class:
			{
				...
				if ( ! base )
				{
					printf( "USB2Analyzer : Error missing class pointer" );
					goto bailout;
				}
				...
				if ( *base == NULL )
				{
					...
					if ( ! IIntuition )
					{
						printf( "USB2Analyzer : Error need Intuition" );
						goto bailout;
					}
...
//					MYINFO( "USB2Analyzer : Opening class '%s'", name );

					ptr = OpenClass( name, vers, cls );

					if ( ! ptr )
					{
						printf( "USB2Analyzer : Error opening class '%s' pos #%u", name, pos );
						goto bailout;
					}
			...
			default:
			{
				printf( "USB2Analyzer : Unknown Resource type (%u)", OpenList[pos].os_Type );
				goto bailout;
			}
		}
	}
...
//	MYERROR( "USB2Analyzer : myCloseResources" );
	...
	{
		pos--;
//		MYERROR( "USB2Analyzer : Processing : %s", OpenList[pos].os_STR );
