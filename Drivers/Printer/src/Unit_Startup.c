...
S32 sig;
	...
	if ( ! unit )
	{
		USBERROR( "Printer : Unit_Startup : NULL Pointer" );
		goto bailout;
	}

	USBDEBUG( "Printer : Unit_Startup : UnitNr #%lu", unit->unit_UnitNr );
	...
	if ( CreateNewProcTags( 
		...
		NP_Entry,		Printer_Process_Entry,
		NP_UserData,	& msg,
		TAG_END ))
	{
		while( TRUE )
	...
	if ( ! msg.Result )
	{
		USBERROR( "Printer : Failed to Printer process" );
		goto bailout;
	}
...
bailout:
	...
	USBDEBUG( "Printer : Unit_Startup : Leave : Retval %ld", retval );
