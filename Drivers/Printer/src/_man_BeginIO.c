...
struct PrinterUnit *unit;
	...
	USBDEBUG( "Printer : _manager_BeginIO : IOReq %p", ioreq );
...
//	ioreq->req_Public.io_Actual	= 0;
	...
	switch( ioreq->io_Command )
	{
		...
		case /* 0003 */	CMD_WRITE:
		{
			...
			if (( ! unit->unit_Shutdown ) && ( unit->unit_StartupComplete ))
			{
				PutMsg( unit->unit_PRT_Begin_MsgPort, (PTR) ioreq );
			}
			else
			{
		...
		default:
		{
			USBDEBUG( "Printer : _manager_BeginIO : Unknown Cmd ($%04lx)", (U32) ioreq->io_Command );
