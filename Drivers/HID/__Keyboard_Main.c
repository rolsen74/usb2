...
SEC_CODE void __Keyboard_Main( struct USBBase *usbbase, PTR userdata, PTR in UNUSED )
{
...
U32 wait;
	...
	TASK_NAME_ENTER( "__Keyboard_Main" );
	...
	while( rn->Running )
	{
		...
		if ( mask &	rn->IOReq_MsgPort.ump_Signal.sig_Signal_Mask )
		{
			while( TRUE )
			{
				...
				__Keyboard_Data( usbbase, rn, ioreq );
